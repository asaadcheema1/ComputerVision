        ...
        if args.render:
            ...
            if args.type == 'format' and '**kern' not in outputs:
                if '\\begin{tikzpicture}' not in outputs:
                    ...
                    for out in outputs_list:
                        if out:
                            if ('\\begin{tikzpicture}' not in out
...
def run_ocr(
    ...
    A wrapper function that:
      1. Constructs a mock args object
      2. Calls eval_model(args)
      3. Captures console output
      4. Returns the OCR output (as a string)
    """
    ...
    # Use contextlib to capture stdout from eval_model
    import io
    ...
    # The full text that was printed by eval_model
    ocr_output = buffer.getvalue()

    # Return that text (only)
    return ocr_output
